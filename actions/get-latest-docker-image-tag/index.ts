 ...
 */
function validateInputs(repo: string, dockerUsername: string, dockerhubToken: string): void {
...
function envOrInput(name: string): string {
...
function getLogger() {
    ...
    return {
        debug: (...msgs: any[]) => core.debug(msgs.map(m => typeof m === 'string' ? m : JSON.stringify(m)).join(' ')),
        info: (...msgs: any[]) => core.info(msgs.map(m => typeof m === 'string' ? m : JSON.stringify(m)).join(' ')),
        error: (...msgs: any[]) => core.error(msgs.map(m => typeof m === 'string' ? m : JSON.stringify(m)).join(' '))
    };
}
...
async function setOutput(name: string, value: any): Promise<void> {
